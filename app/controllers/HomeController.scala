 ...
 */
@Singleton
class HomeController @Inject()(@Named("moviesActor") moviesActor: ActorRef,
  ...
  def message() = Action(BodyParsers.parse.json) { implicit  request =>
    logger.info(s"New movie message")

    implicit val movieMessageReads: Reads[MovieMessage] = (
        (JsPath \ "id").read[String] and
        (JsPath \ "message").read[String]
      )(MovieMessage.apply _)

    val movieMessResult = request.body.validate[MovieMessage]
    movieMessResult.fold(
      errors => {
        BadRequest(Json.obj("status" ->"KO", "message" -> JsError.toJson(errors)))
      },
      movieMess => {
        implicit val timeout = Timeout(100.millis)
        moviesActor ? movieMess
        NoContent
      }
    )
  }
  ...
  def createWebSocketFlow(webSocketIn: Publisher[JsValue], userActor: ActorRef): Flow[JsValue, JsValue, NotUsed] = {
    ...
    val flowWatch: Flow[JsValue, JsValue, NotUsed] = flow.watchTermination() { (_, termination) =>
      termination.foreach { done =>
        ...
        moviesActor.tell(UnSubscibeMovie(None), userActor)
